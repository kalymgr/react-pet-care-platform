...
export const LostPetsInfo = (
    state = { isLoading: true, errMess: null, lostPetsInfo: [], pageNumber: 1}, action
) => {
    switch (action.type) {
        case ActionTypes.ADD_LOSTPETSINFO :  // add a list of lost pets
            return { ...state, isLoading: false, errMess: null, lostPetsInfo: action.payload.lostPetsInfo, pageNumber: action.payload.pageNumber }
